...
export default function RecentActivity() {
  return (
    <div className="mt-8">
      ...
      <div className="space-y-8 relative before:absolute before:left-[11px] before:top-2 before:bottom-0 before:w-px before:bg-gray-200">
        {ACTIVITIES.map((activity) => (
          <div key={activity.id} className="relative pl-8">
            <div className="absolute left-0 top-0 w-6 h-6 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center z-10">
      ...
      <button className="w-full mt-8 py-2 border border-gray-200 rounded-md text-sm font-medium text-blue-600 hover:bg-gray-50 transition-colors">
        Show more activity
      </button>
    </div>
  );
